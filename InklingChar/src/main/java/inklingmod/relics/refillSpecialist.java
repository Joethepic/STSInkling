...
public class refillSpecialist extends BaseRelic {
    private static final String NAME = "refillSpecialist"; //The name will be used for determining the image file as well as the ID.
    public static final String ID = makeID(NAME); //This adds the mod's prefix to the relic ID, resulting in modID:MyRelic
    private static final RelicTier RARITY = RelicTier.RARE; //The relic's rarity.
    private static final LandingSound SOUND = LandingSound.CLINK; //The sound played when the relic is clicked.

    public refillSpecialist() {
        super(ID, NAME, InklingCharacter.Meta.CARD_COLOR, RARITY, SOUND);
    }

    public void onUseCard(AbstractCard card, UseCardAction action) {
        {
            if (card.hasTag(Tags.GAINS_INK)) {
    ...
    {
        return "Gives 1 #yinklingchar:Ink whenever using a card that gains #yinklingchar:Ink.";
    }
