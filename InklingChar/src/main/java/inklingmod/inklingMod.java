...
import inklingmod.cards.BaseCard;
import inklingmod.character.InklingCharacter;
import inklingmod.potions.BasePotion;
import inklingmod.relics.BaseRelic;
import inklingmod.util.GeneralUtils;
import inklingmod.util.KeywordInfo;
import inklingmod.util.Sounds;
...
public class inklingMod implements
    ...
    public static void initialize() {
        new inklingMod();
        logger.info("Entered initialize method");
        InklingCharacter.Meta.registerColor();
    }
    public inklingMod() {
    ...
    private static String checkResourcesPath() {
        String name = inklingMod.class.getName(); //getPackage can be iffy with patching, so class name is used instead.
        ...
        if (!resources.exists()) {
            throw new RuntimeException("\n\tFailed to find resources folder; expected it to be at  \"resources/" + name + "\"." +
                    ...
                    "\t\"private static final String resourcesFolder = checkResourcesPath();\"\n" +
                    "\tat the top of the " + inklingMod.class.getSimpleName() + " java file.");
        }
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo) -> {
            ...
            return initializers.contains(inklingMod.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
    ...
    {
        logger.info("Entered receiveEditCharacters");
        InklingCharacter.Meta.registerCharcter();
    }
    @Override
